...
This is the bank for GPU fitted results. The bank starts with ``unsigned int" that is the number of segments. Then for each of those segments there is a block like this:
an ``unsigned int" which is the number of pulses and then \textit{nPulses} blocks of \textit{pulseFinderResult} struct. The struct is shown in Tab.~\ref{tab:cftable}.
...
\hline
13	& Int\_t	& peak\_value &  value of the fitted peak \\ 
\hline
\end{tabular} 
\label{tab:cftable}
\end{table}
